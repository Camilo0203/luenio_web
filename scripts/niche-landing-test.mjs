...
pages.forEach(({ slug, demo, hook, primaryCta, niche }) => {
  ...
  assert(html.includes("You're losing customers right now without noticing."), `${slug} page must create immediate urgency and loss awareness.`);
  assert(html.includes(primaryCta), `${slug} page must include an action-oriented primary CTA.`);
  assert(html.includes("See the live system"), `${slug} page must include a live-system demo preview CTA.`);
  assert(html.includes("Activate this for my business"), `${slug} page must include the final conversion CTA.`);
  assert(!html.includes("Ver demo en vivo"), `${slug} page must not use passive view-demo CTAs.`);
  assert(!html.includes("Ver cómo funciona"), `${slug} page must not use passive exploration CTAs.`);
  assert(html.includes(`href="${demo}"`), `${slug} page must link directly to ${demo}.`);
  assert(html.includes("niche-problem"), `${slug} page must include a pain-focused problem section.`);
  assert(html.includes("Solución"), `${slug} page must include a Luenio value section.`);
  assert(html.includes("niche-demo-preview"), `${slug} page must include a live demo preview section.`);
  ...
  assert(html.includes("Resultado ejemplo"), `${slug} page must mark sample outcome metrics clearly.`);
  assert(html.includes("Testimonio de muestra"), `${slug} page must mark sample testimonials clearly.`);
  assert(niche.every((term) => html.toLowerCase().includes(term.toLowerCase())), `${slug} page must include niche-specific sales language.`);
});
...
["nicheGym", "nicheRestaurants", "nicheRealEstate", "nicheEcommerce", "nicheAgencies"].forEach((entryName) => {
  assert(viteSource.includes(entryName), `Vite build must include ${entryName}.`);
});

const css = readText("apps/web/src/style.css");
[".niche-page", ".niche-hero", ".niche-loss-line", ".niche-preview-card", ".niche-final-cta"].forEach((selector) => {
