...
process.env.STRIPE_WEBHOOK_SECRET = process.env.STRIPE_WEBHOOK_SECRET || "whsec_luenio_local_test_secret";
...
function assertNoStore(response, label) {
  ...
  assert(response.headers.get("pragma") === "no-cache", `${label} must include legacy no-cache pragma.`);
}
...
function seedStarterLimitLeads(userId, existingLeadId) {
  ...
  fs.writeFileSync(localDbPath, JSON.stringify({ ...database, leads, updatedAt: timestamp }, null, 2));
}
...
try {
  ...
  assert(legacyFolderRoute.body.includes("Luenio Agency"), "Legacy folder route must render the public SaaS page.");
  ...
  assert(Array.isArray(health.body.readiness?.checks), "Health response must include production readiness checks.");
  assert(typeof health.body.readiness?.criticalReady === "boolean", "Readiness must expose criticalReady boolean.");
  assert(!health.body.storage.missing, "Public health must not expose detailed missing environment variables.");
  assert(!health.body.readiness.security, "Public health must not expose security configuration details.");
  assert(!health.body.readiness.billing, "Public health must not expose billing configuration details.");
  ...
  assert(unknownApi.body.error === "API route not found.", "Unknown API routes must return a JSON error.");
  ...
  assert(unsupportedMediaType.response.status === 415, "Mutable API routes must require application/json.");
  ...
  assert(oversizedBody.body.error === "Request body too large.", "Oversized body errors must be explicit.");
  ...
  assert(publicInquiry.response.ok, `Public contact capture failed: ${JSON.stringify(publicInquiry.body)}`);
  assert(publicInquiry.body.inquiryId, "Public contact capture must return inquiry id.");
  assert(publicInquiry.body.inquiryId !== "client_public_inquiry_id", "Public contact capture must generate server-owned inquiry ids.");
  assert(publicInquiry.body.inquiryId.startsWith("inquiry_"), "Public inquiry ids must use the inquiry prefix.");
  assert(publicInquiry.body.webhook?.status === "not_configured", "Public contact capture must report webhook delivery status.");
  assert(!("destination" in (publicInquiry.body.webhook || {})), "Public contact response must not expose webhook destination URLs.");
  assert(!("error" in (publicInquiry.body.webhook || {})), "Public contact response must not expose internal webhook errors.");
  ...
  assert(duplicatePublicInquiry.response.status === 409, "Recent duplicate public inquiries must be rejected.");
  ...
  assert(blockedCrossOrigin.response.status === 403, "Cross-origin mutable API requests must be blocked.");
  ...
  assert(blockedCrossSiteFetch.response.status === 403, "Browser cross-site mutable API requests must be blocked with Fetch Metadata.");
  assert(blockedCrossSiteFetch.body.error === "Cross-site request blocked.", "Fetch Metadata block must return an explicit error.");
  ...
  assert(allowedSameOrigin.response.ok, `Same-origin mutable API request should pass: ${JSON.stringify(allowedSameOrigin.body)}`);
  ...
  assert(authPage.body.includes("Accede a tu CRM de automatización."), "Auth page must render UTF-8 copy correctly.");
  assert(authPage.body.includes("auth-trust"), "Auth page must render workspace trust signals.");
  assert(authPage.response.headers.get("x-frame-options") === "DENY", "Auth page must deny framing.");
  assert(authPage.response.headers.get("x-content-type-options") === "nosniff", "Auth page must set nosniff.");
  assert(authPage.response.headers.get("cache-control") === "no-store", "Auth page must not be cached.");
  ...
  assert(loginPage.body.includes("Accede a tu CRM de automatización."), "Login route must render the auth page.");
  assert(loginPage.response.headers.get("cache-control") === "no-store", "Login route must not be cached.");
  ...
  assert(protectedAdmin.response.headers.get("location") === "/login", "Unauthenticated admin redirect must target /login.");
  assert(protectedAdmin.response.headers.get("cache-control") === "no-store", "Protected admin redirect must not be cached.");

  const protectedDashboard = await unauthenticatedRequest("/dashboard", { redirect: "manual" });
  assert(protectedDashboard.response.status === 302, "Dashboard route must redirect when unauthenticated.");
  assert(protectedDashboard.response.headers.get("location") === "/login", "Unauthenticated dashboard redirect must target /login.");
  ...
  assert(invalidPlanRegistration.response.status === 400, "Registration must reject unknown plans.");
  assert(invalidPlanRegistration.body.error === "Unknown plan.", "Unknown plan registration must return clear error.");
  ...
  assert(created.body.leadId !== "client_owned_crm_lead_id", "Lead API must generate server-owned lead ids.");
  assert(created.body.leadId.startsWith("lead_"), "Lead API ids must use the lead prefix.");
  assert(created.body.classification === "hot", `Expected hot lead, got ${created.body.classification}.`);
  assert(created.body.score >= 80, `Expected score >= 80, got ${created.body.score}.`);
  assert(["contacted", "converted"].includes(created.body.pipelineStage), `Unexpected pipeline stage ${created.body.pipelineStage}.`);
  assert(created.body.usage?.used === 1, `Monthly usage must count the created CRM lead, got ${JSON.stringify(created.body.usage)}.`);
  assert(created.body.usage?.remaining === 99, "Starter monthly usage must report remaining leads.");
  assert(!created.body.actions.includes("send_crm_webhook"), "Starter plan must restrict CRM webhook action.");
  assert(created.body.restrictedActions.includes("send_crm_webhook"), "Restricted CRM webhook action must be reported.");
  assert(created.body.actions.includes("create_crm_deal"), "Automation must still create CRM deal.");
  ...
  assert(spoofedScoreLead.response.ok, `Spoofed score lead should still be processed safely: ${JSON.stringify(spoofedScoreLead.body)}`);
  assert(spoofedScoreLead.body.score < 80, "Lead API must ignore client-provided score.");
  assert(spoofedScoreLead.body.classification !== "hot", "Lead API must ignore client-provided classification.");
  assert(spoofedScoreLead.body.pipelineStage !== "converted", "Lead API must ignore client-provided initial pipeline stage.");
  assert(!spoofedScoreLead.body.scoreReasons.includes("client_score"), "Lead API must ignore client-provided score reasons.");
  assert(spoofedScoreLead.body.usage?.used === 2, "Monthly usage must count safely processed spoofed lead.");
  ...
  assert(storedLead.userId === registered.body.user.id || storedLead.user_id === registered.body.user.id, "Lead must persist inside current tenant.");
  assert(storedLead.source === "hero", "Source tracking must persist.");
  assert(storedLead.score === created.body.score, "Score must persist.");
  const storedSpoofedLead = list.body.leads.find((lead) => lead.id === spoofedScoreLead.body.leadId);
  assert(storedSpoofedLead, "Safely processed spoofed lead must still be persisted.");
  assert(storedSpoofedLead.score === spoofedScoreLead.body.score, "Persisted spoofed lead must use server-computed score.");
  assert(storedSpoofedLead.pipelineStage !== "converted" && storedSpoofedLead.pipeline_stage !== "converted", "Persisted spoofed lead must use server-computed pipeline stage.");
  assert(list.body.events.some((event) => event.type === "lead.created"), "Lead creation must record an event.");
  const lifecycleEvents = ["message.received", "intent.classified", "crm.updated", "followup.triggered", "automation.triggered"];
  lifecycleEvents.forEach((eventType) => {
    assert(list.body.events.some((event) => event.type === eventType), `Lead lifecycle must record ${eventType}.`);
  });
  const storedAction = list.body.actions.find((action) => action.leadId === created.body.leadId || action.lead_id === created.body.leadId);
  assert(storedAction, "Created lead must persist an automation action log.");
  const integrationResults = storedAction.integrationResults || storedAction.integration_results || [];
  integrationResults.forEach((result) => {
    assert(!("destination" in result), "Persisted automation integration results must not expose destination URLs.");
    assert(!("error" in result), "Persisted automation integration results must not expose raw provider errors.");
  });
  const restrictedActions = storedAction.restrictedActions || storedAction.restricted_actions || [];
  assert(restrictedActions.includes("send_crm_webhook"), "Restricted automation actions must persist for upgrade prompts.");
  ...
  assert(secondRegistered.response.ok, `Second tenant registration failed: ${JSON.stringify(secondRegistered.body)}`);

  const secondTenantLeads = await request("/api/leads");
  assert(secondTenantLeads.response.ok, `Second tenant lead list failed: ${JSON.stringify(secondTenantLeads.body)}`);
  assert(!secondTenantLeads.body.leads.some((lead) => lead.id === created.body.leadId), "Second tenant must not see first tenant lead.");

  const forbiddenPipelineUpdate = await request("/api/process", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ leadId: created.body.leadId, status: "converted", pipelineStage: "converted" }),
  });
  assert(forbiddenPipelineUpdate.response.status === 404, "Second tenant must not update first tenant lead.");
  ...
  const invalidPipelineUpdate = await request("/api/process", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ leadId: created.body.leadId, status: "lost_forever", pipelineStage: "lost_forever" }),
  });
  assert(invalidPipelineUpdate.response.status === 400, "Invalid pipeline stage updates must be rejected.");
  ...
  assert(updatedLead.status === "converted" || updatedLead.pipeline_stage === "converted", "Pipeline status must update to converted.");
  assert(afterUpdate.body.events.some((event) => event.type === "pipeline.updated"), "Pipeline update must record an event.");

  const admin = await request("/admin.html");
  assert(admin.response.ok && admin.body.includes("pipelineBoard"), "Admin dashboard must render pipeline board.");
  assert(admin.response.headers.get("cache-control") === "no-store", "Admin dashboard must not be cached.");
  ...
  assert(admin.body.includes("billingStatus"), "Admin dashboard must render billing status feedback.");
  const adminJs = fs.readFileSync(path.join(process.cwd(), "apps", "admin", "src", "admin.js"), "utf8");
  assert(adminJs.includes("loadDashboard({ silent: true, showSkeleton: false })"), "Background CRM polling must refresh silently without skeleton flicker.");
  assert(adminJs.includes("state.loadedOnce"), "Admin dashboard must track initial load state.");

  const dashboard = await request("/dashboard");
  assert(dashboard.response.ok && dashboard.body.includes("pipelineBoard"), "Clean /dashboard route must render pipeline board for authenticated users.");
  assert(dashboard.response.headers.get("cache-control") === "no-store", "Dashboard route must not be cached.");
  ...
  assert(billing.body.usage?.used === 2, `Billing usage must be monthly and tenant-scoped, got ${JSON.stringify(billing.body.usage)}.`);
  assert(billing.body.usage?.limit === 100, "Billing usage must include active plan monthly limit.");
  ...
  assert(invalidCheckoutPlan.response.status === 400, "Billing checkout must reject unknown plans before Stripe.");
  assert(invalidCheckoutPlan.body.error === "Unknown plan.", "Unknown billing plans must return a clear error.");
  ...
  assert(normalizedStageLead.response.ok, `Lead processing with invalid client stage should normalize: ${JSON.stringify(normalizedStageLead.body)}`);
  assert(["new", "qualified", "contacted", "converted"].includes(normalizedStageLead.body.lead.pipelineStage), "Lead processing must normalize invalid pipeline stages.");
  ...
  if (health.body.storage.mode === "json_fallback") {
    ...
    assert(blockedProcessLead.response.status === 402, "Full lead processing must enforce the monthly plan limit.");
    assert(blockedProcessLead.body.usage?.used === 100, "Blocked process response must include current monthly usage.");
  }

  const settings = await request("/api/settings");
  assert(settings.response.ok, `Settings API failed: ${JSON.stringify(settings.body)}`);
  assert(settings.body.checklist?.length >= 2, "Settings checklist must include workspace readiness items.");
  assert(settings.body.readiness?.checks?.length >= 6, "Settings must include production readiness checks.");
  assert(settings.body.readiness.checks.some((check) => check.id === "stripe_prices"), "Readiness must check Stripe price IDs.");
  assert(settings.body.readiness.security, "Authenticated settings must include security readiness details.");
  assert(settings.body.readiness.billing, "Authenticated settings must include billing readiness details.");
  assert(settings.body.readiness.deployment, "Authenticated settings must include deployment readiness details.");
  ...
  assert(badStripeWebhook.response.status === 400, "Malformed Stripe signatures must be rejected with 400.");
  ...
  assert(afterBilling.body.events.some((event) => event.type === "subscription.updated"), "Stripe webhook must record subscription event.");
  ...
  assert(loggedOutSession.body.authenticated === false, "Session must be unauthenticated after logout.");
