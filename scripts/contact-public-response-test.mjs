...
try {
  ...
  assert(response.statusCode === 200, `Public contact handler should accept valid inquiry, got ${response.statusCode}.`);
  assert(deliveredTo === process.env.CONTACT_WEBHOOK_URL, "Configured contact webhook should still be invoked internally.");
  assert(response.body.webhook?.status === "failed", "Public response should report sanitized webhook status.");
  assert(response.body.webhook?.httpStatus === 502, "Public response may expose HTTP status without internal destination.");
  assert(!("destination" in response.body.webhook), "Public response must not expose contact webhook destination.");
  assert(!("error" in response.body.webhook), "Public response must not expose contact webhook error details.");
