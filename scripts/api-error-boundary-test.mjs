import { getErrorStatus, getPublicErrorMessage, sendApiError } from "../api/services/http-response.js";
...
assert(getPublicErrorMessage(internalError, 500) === "Internal server error.", "5xx errors must not expose raw internals.");

const validationError = new Error("Missing required fields");
validationError.statusCode = 400;
assert(getPublicErrorMessage(validationError, 400) === "Missing required fields", "4xx operational errors must stay actionable.");
...
assert(response.body.error === paidLimitError.message, "sendApiError must preserve 4xx operational messages.");
...
assert(unsafeResponse.body.error === "Internal server error.", "sendApiError must hide internal 5xx messages.");
