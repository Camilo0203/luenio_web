...
try {
  ...
  globalThis.fetch = async (url, options) => {
    assert(
      ...
      "Checkout must call Stripe sessions API.",
    );
  ...
  assert(
    checkout.id === "cs_test_checkout_guard",
    "Checkout response must pass through Stripe session id.",
  );
  assert(
    stripePayload.get("success_url") === "https://luenio.example/dashboard?billing=success",
    "Checkout success URL must use production APP_URL origin.",
  );
  assert(
    stripePayload.get("cancel_url") === "https://luenio.example/dashboard?billing=cancelled",
    "Checkout cancel URL must use production APP_URL origin.",
  );
  assert(
    stripePayload.get("success_url").includes("attacker.example") === false,
    "Checkout URLs must not trust request Host in production.",
  );
